...
module Datapath(Clock,Reset,
	...
	assign md_bridge = (EM_alu>=32'h0000&&EM_alu<=32'h2fff)?md:PrRd; // new
	...
	IF Ifu(.Branch_Jump(D_Branch_Jump&&(~E_HWClr)),.Enable(~Stall),.Clock(Clock),.Reset(Reset),.Handler(Handler),.PC_Update(pc_update),
	...
	// BED
	BED Bed(.StoreType(M_Store_Type),.Addr(EM_alu[1:0]),.ByteEnable(byteenable));
	// DM
	DM Dm(.MemWrite(M_MemWrite&&(~WriteProtect)),.MemRead(M_MemRead),.Load_Type(M_Load_Type),.SignRead(M_SignRead),
			.Clock(Clock),.Reset(Reset),
			.ByteEnable(byteenable),.WPC(EM_pc),.Addr(EM_alu),.WD(tmux_em_rd2),
			.RD(md));
	...
	/*// Bridge module Bridge(Clock,Reset,PrWe,PrBE,PrAddr,PrWD,PrRd,HWInt);
	Bridge bridge(.Clock(Clock),.Reset(Reset),.PrWe(M_MemWrite&&(~WriteProtect)),.PrBE(byteenable),.PrAddr(EM_alu),.PrWd(tmux_em_rd2),
